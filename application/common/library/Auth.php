...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'telephone', 'avatar', 'score'];
     ...
     * @return boolean
     */
    public function register($username, $password, $email = '', $telephone = '', $extend = [])
    {
        // 检测用户名或邮箱、手机号是否存在
        ...
        if ($email && model("user")->getByEmail($email))
        {
            $this->setError('Email already exist');
            return false;
        }
        if ($telephone && model("user")->getByMobile($telephone))
        {
        ...
        try
        {
            $user = model("user")->create($params);
            Db::commit();

            // 此时的Model中只包含部分数据
            $this->_user = model("user")->get($user->id);
    ...
    {
        $field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^1\d{10}$/') ? 'telephone' : 'username');
        $user = model("user")->get([$field => $account]);
        if (!$user)
        {
    ...
    {
        $user = model("user")->hasWhere('genearch',['wxopenid'=>$openid])->find();
        if (!$user)
        {
            $this->setError('Account is incorrect');
            return FALSE;
