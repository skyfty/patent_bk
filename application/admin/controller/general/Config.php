...
class Config extends Backend
{
    ...
    {
        $config = $this->formatConfig('basic');
        $configgroup = $this->model->group("group")->column("group");
        foreach($configgroup as $k=>$v) {
            $config['configgroup'][$v] = $v;
        }
        file_put_contents(APP_PATH . 'extra' . DS . 'site.php', '<?php' . "\n\nreturn " . var_export($config, true) . ";");

        foreach($configgroup as $group) {
            if ($group == "basic")
                continue;
            $config = $this->formatConfig($group);
            $export = var_export($config, true);
            $phpscr = '<?php' . "\n\nreturn " . $export . ";";
            file_put_contents(APP_PATH . 'extra' . DS . $group.'.php', $phpscr);
        }
    }

    private function formatConfig($group) {
        $config = [];
        foreach ($this->model->all(['group'=>$group]) as $k => $v) {
            $value = $v->toArray();
