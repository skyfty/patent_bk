...
class Archives extends Backend
{
    ...
    public function edit($ids = NULL)
    {
        ...
        $channel = \app\admin\model\Channel::get($row['channel_id']);
        if (!$channel) {
            $this->error(__('No specified channel found'));
        }

        $disabledIds = [];
        $all = collection(\app\admin\model\Channel::order("weigh desc,id desc")->select())->toArray();
        foreach ($all as $k => $v) {
    ...
    public function del($ids = "")
    {
        \app\admin\model\Archives::event('after_delete', function ($row) {
            \app\admin\model\Channel::where('id', $row['channel_id'])->where('items', '>', 0)->setDec('items');
        });
    ...
    public function restore($ids = "")
    {
        ...
        if ($count) {
            ...
            foreach ($channelNums as $k => $v) {
                \app\admin\model\Channel::where('id', $k)->setInc('items', $v);
            }
            $this->success();
        }
    ...
    public function move($ids = "")
    {
        if ($ids) {
            ...
            $this->model->where($pk, 'in', $ids);
            $channel = \app\admin\model\Channel::get($channel_id);
            if ($channel && $channel['type'] === 'list') {
                ...
                if ($result) {
                    ...
                    foreach ($channelNums as $k => $v) {
                        if ($v['channel']) {
                            \app\admin\model\Channel::where('id', $v['channel_id'])->where('items', '>', 0)->setDec('items', min($v['channel']['items'], $v['nums']));
                        }
                        $count += $v['nums'];
                    }
                    \app\admin\model\Channel::where('id', $channel_id)->setInc('items', $count);
                    $this->success();
                } else {
                    $this->error(__('No rows were updated'));
                }
    ...
    public function get_channel_fields()
    {
        ...
        $channel = \app\admin\model\Channel::get($channel_id, 'model');
        if ($channel && $channel['type'] === 'list') {
            $values = [];
            if ($archives_id) {
