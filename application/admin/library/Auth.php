 ...
 */
    public function login($username, $password, $branch = null, $keeptime = 0) {
        if ($branch != null) {
            $admin = model("admin")->with(["staff"])->where("staff.branch_model_id", $branch)->where('username', $username)->find();
        } else {
            $admin = null;
        }
        if ($admin == null){
            $admin = model("admin")->where('username', $username)->whereNull('staff_id')->find();
        }
    ...
    public function checka($name)
    {
        return parent::check($name, $this->id, 'or', '');
    }
     ...
     * @return array
     */
    public function getChildrenGroupIds($withself = false, $branch = null)
    {
        //取出当前管理员所有的分组
        ...
        $where = ['status' => 'normal'];
        if ($branch !== null) {
            $where['branch_id'] = ['in',[-1,$branch]];
        }
        $groupList = \app\admin\model\AuthGroup::where($where)->select();
        $objList = [];
        foreach ($groups as $K => $v) {
            if ($v['rules'] === '*') {
                $objList = $groupList;
    ...
    public function getChildrenDepartmentIds($id, $pid) {
        // 取出所有分组
        $departmentList = \app\admin\model\Department::all();
        $objList = [];

        // 取出包含自己的所有子节点
        $childrenList = Tree::instance()->init($departmentList)->getChildren($id, true);
