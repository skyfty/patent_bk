...
class Channel Extends Model
{
    ...
    {
        return isset($data['type']) && isset($data['outlink']) && $data['type'] == 'link' ? $data['outlink'] : url('archives/index', ['id' => $data['id']]);
    }

    public function getFullurlAttr($value, $data)
    {
        return isset($data['type']) && isset($data['outlink']) && $data['type'] == 'link' ? $data['outlink'] : url('archives/index', ['id' => $data['id']], true, true);
    }

    public function getImageAttr($value, $data)
    {
        $value = $value ? $value : Config::get('default_channel_img');
        return cdnurl($value);
    }
    ...
    {
        $list = [];
        $list[] = ['name' => __('Home'), 'url' => 'cms/index/index'];
        if ($channel) {
