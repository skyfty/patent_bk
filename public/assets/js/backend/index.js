        ...
        index: function () {
            ...
            $.ajax({url: '/note/index', type: 'get',
                ...
                success: function (data) {
                    $(".notifications-menu > a span").text(data.rows > 0 ? data.rows.length : '');
                    $(".notifications-menu .footer a").attr("href", "note/index");
                    $.each(data.rows, function (i, j) {
                        var item = '<li><a href="note/view/ids/' + j.id + '"  class="btn-dialog" title="查看公告" target="_blank">' + j.title + '</a></li>';
