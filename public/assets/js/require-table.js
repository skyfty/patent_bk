define(['jquery', 'bootstrap', 'moment', 'moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-export', 'bootstrap-table-commonsearch', 'bootstrap-table-template','bootstrap-treegrid'], function ($, undefined, Moment) {
    var Table = {
        list: {},
        ...
        api: {
            init: function (defaults, columnDefaults, locales) {
                ...
                BootstrapTable.prototype.resetTableTips = function () {
                    var that = this;
                    require(['qtip2'], function (qtip2) {
                        $('thead tr th[data-field].number-total', that.$el).qtip("destroy");
                        $('thead tr th[data-field].number-total', that.$el).qtip({
                            style: {
                                classes: 'qtip-bootstrap',
                            },
                            content: {
                                text: function(event, api) {
                                    var url = that.options.extend['summation_url'];
                                    var params = that.options.queryParams({});
                                    var field = $(this).attr("data-field");
                                    if ($.isFunction(url)) {
                                        var defer = url(params, field);
                                        ...
                                        $.ajax({
                                            ...
                                            api.set('content.text', "合计: " + content.data);
                                        });
                                    }
                                    return 'Loading...';
                                }
                ...
                $.fn.bootstrapTable.methods.push('resetTableTips');
            },
            // 绑定事件
            bindevent: function (table) {
                ...
                table.on('post-body.bs.table', function (e, settings, json, xhr) {
                    ...
                    if ($(Table.config.firsttd, table).find("input[type='checkbox'][data-index]").size() > 0) {
                        ...
                        $.each(table.bootstrapTable('getData'), function (i, j) {
                            if (j["status"] == "locked") {
                                var sel = "input[type='checkbox'][data-index='"+ i+"']";
                                $(sel, table).attr("disabled","disabled");
                            }
                        });
                    }
                });
                ...
                $(table).on("click", "[data-id].btn-view", function (e) {
                    e.preventDefault();
                    var ids = $(this).data("id");
                    var row = Table.api.getrowbyid(table, ids);
                    row.ids = ids;
                    var url = Table.api.replaceurl(options.extend.view_url, row, table);
            ...
            multi: function (action, ids, table, element) {
                ...
                var url = options.extend.multi_url;
                if (typeof options.extend[action + "_url"] !== "undefined") {
                    url = options.extend[action + "_url"];
                }
            ...
            formatter: {
                ...
                image: function (value, row, index) {
                    ...
                    return '<a href="' + value + '" target="_blank" class="preview"><img class="' + classname + '" src="' + value + '" /></a>';
                },
                images: function (value, row, index) {
                    ...
                    $.each(arr, function (i, value) {
                        value = value ? value : '/assets/img/blank.gif';
                        html.push('<a  href="' + value + '" target="_blank" class="preview"><img class="' + classname + '" src="' + Fast.api.cdnurl(value) + '" /></a>');
                    });
                    return html.join(' ');
                },
                ...
                video: function (value, row, index) {
                    return Table.api.formatter.videos.call(this, value, row, index);
                },
                videos: function (value, row, index) {
                    var html = [];
                    value = value === null ? '' : value.toString();
                    if (value != "") {
                        var filemap = {
                        ...
                        $.each(arr, function (i, value) {
                            ...
                            html.push('<a href="/index/player?url=' + Fast.api.cdnurl(value) + '" target="_blank" title="' + Fast.api.cdnurl(value) + '" class="btn-dialog" alt="' + Fast.api.cdnurl(value) + '"><i class="fa ' + exticon + '"></i></a>');
                        });
                    }
                    return html.join(' ');
                },
                ...
                flag: function (value, row, index) {
                    ...
                    if (typeof row.extend !== 'undefined' && row.extend) {
                        var extend = eval('(' + row.extend + ')');
                        colorArr = $.extend(colorArr, extend);
                    }
		            var field = this.field;
                    if (typeof this.customField !== 'undefined' && typeof row[this.customField] !== 'undefined') {
                        value = row[this.customField];
                        field = this.customField;
                    } else if (typeof(field) == "undefined" && typeof row['name'] !== 'undefined') {
                        field = row['name'];
                    }
                ...
                datetime: function (value, row, index) {
                    ...
                    if (isNaN(value)) {
                        ...
                        if (mom.unix() === 0) return "-";
                        return mom.format(datetimeFormat);
                    } else {
                ...
                location: function (value, row, index) {
                    ...
                    if (row.location_lat && row.location_lng) {
                        ...
                        url += '?lat=' + row.location_lat + '&lng=' + row.location_lng + '&range=' + row.range;
                        html = '<a href="'+url+'" class="dialogit" title="' + row.name + '">' + value + '</a>';
                    } else {
                        html = value;
                    }
                    return html
                },
                modelx: function (value, row, index) {
                    var pos  =  this.field.lastIndexOf("_model_id");
                    if (pos == -1)
                        return;
                    var filed = this.field.substr(0, pos);
                    var html = "";
                    if (value) {
                        if (row[filed]) {
                            html = row[filed]['idcode'] + "," + row[filed]['name'];
                        }
                    }
                    return html
                },
            ...
            buttonlink: function (column, buttons, value, row, index, type) {
                ...
                $.each(buttons, function (i, j) {
                    if (type === 'operate') {
                        ...
                        if (['edit', 'del', 'multi'].indexOf(j.name) > -1 && row.status == "locked") {
                            return true;
                        }
                    }
                    ...
                    if (typeof attr === 'undefined' || attr) {
                        ...
                        icon = typeof j.icon === 'function' ? j.icon.call(table, row, j) : j.icon ? j.icon : '';
